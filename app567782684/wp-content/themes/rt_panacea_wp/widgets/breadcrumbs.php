...
class GantryWidgetBreadcrumbs extends GantryWidget {
    ...
    function render($args, $instance){
	    ...
	    <span class="breadcrumbs pathway">
			...
			foreach ($breadcrumbs as $crumb) echo $crumb.'<img alt="" src="'.$gantry->templateUrl.'/images/arrow.png"/>'; 
			
			if(is_single() && intval($instance['category'])===1) :
				$category = get_the_category();
				if(!empty($category)) :
					echo '<a class="pathway" href="'.get_category_link($category[0]->cat_ID).'">'.$category[0]->cat_name.'</a><img alt="" src="'.$gantry->templateUrl.'/images/arrow.png"/>';
				endif;
			endif;
